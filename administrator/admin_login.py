...
# Helper functions for checking initialization state
def has_tables():
    """Check if database tables exist"""
    try:
        import sqlite3
        conn = sqlite3.connect('guidelo.db')
        cursor = conn.cursor()
...
class AdminMainWindow(QtWidgets.QWidget):
    """Enhanced admin main window with stepwise initialization logic"""
    ...
    def setup_ui(self):
        ...
        # Dynamic content area (will be updated based on state)
        self.dynamic_content_layout = QtWidgets.QVBoxLayout()
        main_layout.addLayout(self.dynamic_content_layout)
    ...
    def clear_dynamic_content(self):
        """Clear all widgets from dynamic content area"""
        for i in reversed(range(self.dynamic_content_layout.count())):
            item = self.dynamic_content_layout.itemAt(i)
            if item.widget():
    ...
    def update_ui_based_on_state(self):
        """Update UI based on initialization state"""
        self.clear_dynamic_content()

        # Check current state
        tables_exist = has_tables()
        slots_exist = has_slots()

        if not tables_exist:
            # Step 1: Need to create tables
            self.show_create_tables_section()
        elif not slots_exist:
            # Step 2: Need to initialize slots or import CSV
            self.show_initialize_slots_section()
        else:
            # All done: Show management tools
            self.show_management_section()

    def show_create_tables_section(self):
        """Show create tables section"""
        self.status_indicator.set_status("error", 
            "Step 1: Database tables need to be created before proceeding.", "⚠️")

        step1_card = ModernCard("🗄️ Step 1: Create Database Tables")

        desc_label = QtWidgets.QLabel("Create the required database tables to set up your system:")
        desc_label.setObjectName("sectionDescription")
        step1_card.addWidget(desc_label)
        ...
        step1_card.addWidget(self.create_tables_btn)

        self.dynamic_content_layout.addWidget(step1_card)

    def show_initialize_slots_section(self):
        """Show initialize slots section"""
        self.status_indicator.set_status("warning", 
            "Step 2: Initialize slots or import CSV data to complete setup.", "⚠️")

        step2_card = ModernCard("🎯 Step 2: Initialize Slots")

        desc_label = QtWidgets.QLabel("Choose one option to set up your slot data:")
        desc_label.setObjectName("sectionDescription")
        step2_card.addWidget(desc_label)

        buttons_grid = QtWidgets.QHBoxLayout()
        ...
        buttons_grid.addWidget(self.initialize_slots_btn)

        self.import_csv_btn = ModernButton("📊 Import Master CSV", "secondary")
        self.import_csv_btn.clicked.connect(self.open_import_csv_window)
        buttons_grid.addWidget(self.import_csv_btn)

        step2_card.addLayout(buttons_grid)
        self.dynamic_content_layout.addWidget(step2_card)

    def show_management_section(self):
        """Show management tools section"""
        self.status_indicator.set_status("success", 
            "✅ All initial setup steps completed. System is ready for use.", "✅")

        mgmt_card = ModernCard("⚙️ Management Tools")

        desc_label = QtWidgets.QLabel("System is fully initialized. Access management tools:")
        desc_label.setObjectName("sectionDescription")
        ...
        self.dynamic_content_layout.addWidget(mgmt_card)

    def setup_system_section(self, layout):
        """Setup system section (always visible)"""
    ...
    def reset_system(self):
        ...
        if reply.exec_() == QtWidgets.QMessageBox.Yes:
            try:
                ...
                # Refresh UI to show first-time setup again
                self.update_ui_based_on_state()
